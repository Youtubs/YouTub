...
Server_YouTube = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_YouTube.."Token_DevYouTube") then
...
redis:set(Server_YouTube.."Token_DevYouTube",token)
end 
else
io.write('\n\27[1;31m⌔︙Token was not saved \n لم يتم حفظ التوكن \n\27[0;39;49m')
end 
os.execute('lua YouTube.lua')
end
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_YouTube.."User_DevYouTube1") then
io.write('\n\27[1;35m⌔︙Send ID For Sudo : ارسل ايدي المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo = io.read():gsub('@','')
if User_Sudo ~= '' then
io.write('\n\27[1;31m⌔︙The ID Is Saved : تم حفظ ايدي المطور\n\27[0;39;49m')
redis:set(Server_YouTube.."Id_DevYouTube",User_Sudo)
io.write('\n\27[1;35m⌔︙Send UserName For Sudo : ارسل معرف المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo2 = io.read():gsub('@','')
if User_Sudo ~= '' then
redis:set(Server_YouTube.."User_DevYouTube1",User_Sudo2)
end
else
...
------------------------------------------------------------------------------------------------------------
local DevYouTube_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DevYouTube_Info_Sudo:write([[
do 
local YouTube_INFO = {
Id_DevYouTube = ]]..redis:get(Server_YouTube.."Id_DevYouTube")..[[,
UserName_YouTube = "]]..redis:get(Server_YouTube.."User_DevYouTube1")..[[",
Token_Bot = "]]..redis:get(Server_YouTube.."Token_DevYouTube")..[["
}
return YouTube_INFO
end

]])
DevYouTube_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_YouTube = io.open("YouTube", 'w')
Run_File_YouTube:write([[
#!/usr/bin/env bash
cd $HOME/YouTube
token="]]..redis:get(Server_YouTube.."Token_DevYouTube")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./YouTube.lua -p PROFILE --bot=$token
done
]])
Run_File_YouTube:close()
------------------------------------------------------------------------------------------------------------
local Run_SM = io.open("YT.RUN", 'w')
Run_SM:write([[
#!/usr/bin/env bash
cd $HOME/YouTube
while(true) do
rm -fr ../.telegram-cli
screen -S YouTube -X kill
screen -S YouTube ./YouTube
done
]])
Run_SM:close()
io.popen("mkdir Files")
os.execute('chmod +x tg')
os.execute('chmod +x YouTube')
os.execute('chmod +x YT.RUN')
os.execute('./YT.RUN')
Status = true
else   
f:close()  
redis:del(Server_YouTube.."Token_DevYouTube");redis:del(Server_YouTube.."Id_DevYouTube");redis:del(Server_YouTube.."User_DevYouTube1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_YouTube
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevYouTube
Ids_Dev = {sudos.Id_DevYouTube,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "يوتيوب"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_YouTube(msg)  
local Dev_YouTube = false  
...
function Dev_YouTube_User(user)  
local Dev_YouTube_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_YouTube_User = true  
end  
end  
return Dev_YouTube_User  
end 
function DeveloperBot(msg)  
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_YouTube(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_YouTube(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_YouTube(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_YouTube(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_YouTube(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_YouTube(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_YouTube(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
if Dev_YouTube_User(user_id) then
Status = true  
...
if Dev_YouTube_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/Youtubs-tech/YouTub/main/YouTube.lua
file_path = 'YouTube.lua'
local respbody = {} 
...
url = ''https://raw.githubusercontent.com/Youtubs/YouTub/main/YouTube.lua' 
file_path = 'YouTube.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
data.first_name_ = gmatch or 'YouTube'
end
...
------------------------------------------------------------------------------------------------------------
function FilesYouTube(msg)
File_Bot = dofile("YouTube.lua")
if File_Bot.YouTube and msg then
Text_File = File_Bot.YouTube(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesYouTubeBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.YouTubeFile and msg then
Text_FileBot = Text_FileBot.YouTubeFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_YouTube_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DevYouTube'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DevYouTube'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DevYouTube',text)
redis:del(bot_id..'GetTexting:DevYouTube'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_YouTube(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."YouTube:UsersBot")
local Groups = redis:smembers(bot_id..'YouTube:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'YouTube:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:User"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'YouTube:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'YouTube:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'YouTube:Mod:User'..Groups[i])
...
download_to_file(''https://raw.githubusercontent.com/Youtubs/YouTub/main/YouTube.lua','YouTube.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/YouTubeTeam/Files_YouTube/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس YouTube\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/YouTubeTeam/Files_YouTube/master/Files_YouTube/"..File_Name)
if Res == 200 then
...
if not Dev_YouTube(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_, msg.id_,'- هلو حب ، لايمكنك استخدام البوت 💕.\n- ﭑشترك اولاً 💕 • ['..redis:get(bot_id..'add:ch:username')..'] .')
return false
end
...
redis:set(bot_id..'GetTexting:DevYouTube'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:DevYouTube')
...
if Dev_YouTube_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..YouTube..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_YouTube(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^اضف مطور @(.*)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^حذف مطور @(.*)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^اضف مطور (%d+)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^حذف مطور (%d+)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text == 'جلب نسخه احتياطيه' and Dev_YouTube(msg) or text == 'جلب نسخه الكروبات' and Dev_YouTube(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
if text and text:match("^تغير الاشتراك$") and Dev_YouTube(msg) then  
redis:setex(bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 360, true)  
send(msg.chat_id_, msg.id_, '܁༯┆حسنا ارسل المعرف الان 💞 ܰ') 
return false  
end
if text and text:match("^وضع قناة الاشتراك$") and Dev_YouTube(msg) then  
redis:setex(bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 360, true)  
send(msg.chat_id_, msg.id_, '܁༯┆ارسل معرف قناتك 💞 ܰ') 
return false  
end
if text == "تفعيل الاشتراك" and Dev_YouTube(msg) then  
if redis:get(bot_id..'add:ch:id') then
...
if text == ("مسح قائمه العام") and Dev_YouTube(msg) or text == ("مسح المحظورين عام") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text == ("قائمه العام") and Dev_YouTube(msg) or text == ("المحظورين عام") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if Dev_YouTube_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
if Dev_YouTube_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_YouTube(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text == "تفعيل المغادره" and Dev_YouTube(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_YouTube(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_YouTube(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_YouTube(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_YouTube(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_YouTube(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_YouTube(msg) then
redis:set(bot_id..'GetTexting:DevYouTube'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_YouTube(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_YouTube(msg) then 
...
elseif text == ("ردود المطور") and Dev_YouTube(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_YouTube(msg) then 
...
elseif text == "حذف رد للكل" and Dev_YouTube(msg) then 
...
if Dev_YouTube_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
...
elseif text == "تغير اسم البوت" and Dev_YouTube(msg) or text == "تغيير اسم البوت" and Dev_YouTube(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_YouTube(msg) then 
...
𓂅 .YouTube TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/TV_TV6)

𓂅 . [Source Info ](t.me/TTV_VV)     

𓂅 . [YouTube iNDT](t.me/YouTube_DEV)     
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS YouTube](t.me/gh_gh90bot)     
]]) 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_YouTube(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_YouTube(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_YouTube(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevYouTube = redis:get(bot_id..'Texting:DevYouTube')
if TextingDevYouTube then 
send(msg.chat_id_, msg.id_,TextingDevYouTube)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_YouTube(msg) then
redis:del(bot_id..'Texting:DevYouTube')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_YouTube(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_YouTube_File(msg,data)
FilesYouTubeBot(msg,data)
